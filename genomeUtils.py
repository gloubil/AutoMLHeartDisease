...
def zerosByMeanRestingBP(df : pa.DataFrame):
    mean = float(df['RestingBP'].mean())
    df.loc[df['RestingBP'] == 0, 'RestingBP'] = mean
    return df

def zeroByMedianRestingBP(df : pa.DataFrame):
    median = float(df['RestingBP'].median())
    df.loc[df['RestingBP'] == 0, 'RestingBP'] = median
    return df
...
def zerosByMeanCholesterol(df : pa.DataFrame):
    ...
    df.loc[df['Cholesterol'] == 0.0, 'Cholesterol'] = mean
    return df

def zerosByMedianCholesterol(df : pa.DataFrame):
    df['Cholesterol'] = df['Cholesterol'].astype('float64')
    median : float = float(df['Cholesterol'].median())
    df.loc[df['Cholesterol'] == 0.0, 'Cholesterol'] = median
    return df
...
def knnInferCholesterol(df : pa.DataFrame):
    ...
    try:
        ...
        return doNothingCholesterol(df)
...
def aberrantByMeanOldpeak(df : pa.DataFrame):
    ...
    df.loc[df['Oldpeak'] < lb, 'Oldpeak'] = mean
    df.loc[df['Oldpeak'] > ub, 'Oldpeak'] = mean
...
def aberrantByMedianOldpeak(df : pa.DataFrame):
    ...
    df.loc[df['Oldpeak'] < lb, 'Oldpeak'] = median
    df.loc[df['Oldpeak'] > ub, 'Oldpeak'] = median
